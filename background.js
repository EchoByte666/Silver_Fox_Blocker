...
// ===== 全局配置 =====
// 日志开关：DEBUG=true 时输出详细日志（chrome://extensions 查看 SW 日志）
const DEBUG = false;
const LOG = '[银狐拦截] ';
...
// 远程规则源列表（按优先级依次尝试）。
// 注意：offscreen.js 与 popup.js 中有相同列表，修改时需三处同步。
...
// ===== v2.1.3：编辑距离 ≤1 快速判定（域名仿冒模糊匹配用，与 content.js 同步）=====
// 在首个差异点处穷举插入/删除/替换三种单步操作，任一对齐剩余即距离 ≤1。
// 覆盖 typosquatting 拼写变体：huorrong（双写 r）/ todesc（替换）等
...
// --- 域名注册域提取（RDAP/ICP 查询目标，www.huorrong.com.cn → huorrong.com.cn）---
