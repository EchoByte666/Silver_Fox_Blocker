...
const { queryDomainAge, queryIcpRecord, skipIcpQueryResult, icpNumbersMatch } =
  globalThis.__YH_DOMAIN_INTEL__;
const { ensureUserTrustLoaded, markUserTrusted, isUserTrustedActive,
...
globalThis.__YH_AI_LINK__.init({
  debug: debug,
  refreshWhitelist: function() { return refreshWhitelistCache(); },
  isAllowedTarget: function(url, hostname) {
    return setMatches(cloudWhitelistSet, hostname) ||
      isDefaultWhitelisted(hostname) || matchesLocalWhitelist(url) ||
      isRecentlyBypassed(url) || isGovCn(hostname);
  },
  isBlockedHost: function(hostname) { return setMatches(blocklistSet, hostname); },
  getBrandConfig: function() { return cache.brandConfig; },
  queryIcpRecord: queryIcpRecord
});
globalThis.__YH_DNR__.init({
  getBlocklist: function() { return cache.blocklist; },
...
// v2.7.1：域名情报（RDAP 域龄五通道 + ICP 备案多源查询）已抽取至
// modules/domain-intel.js（顶部 importScripts 解构引入）
...
// v2.7.1：icpNumbersMatch（备案号主体号/序号剥离比对）已随域名情报抽取至
// modules/domain-intel.js
...
// v2.7.1：用户信任记忆（host 级 7 天人工放行信号，storage.local 持久化）
// 已抽取至 modules/user-trust.js（顶部解构引入；TTL/折扣常量随模块导出）
...
// v2.7.1：DNR 动态规则（分批安装/全量替换/串行排队）已抽取至
// modules/dnr-rules.js——黑名单与放行域数据经 init 依赖注入
// ===== 规则加载 =====
...
// v2.7.1：AI 外链核查（结论分级/缓存持久化/可疑信号/品牌仿冒判定/
// ICP 裁决/批次处理）已抽取至 modules/ai-link.js——白名单/黑名单/品牌库
// 等运行时状态经 init 依赖注入，消息路由仍走本文件 handleAiChatLinkScan
