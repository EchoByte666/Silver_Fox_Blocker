...
// v2.7.1：官方标识检测（CONAC 防伪造双通道）与悬浮验证卡片已抽取至
// modules/content/verify-card.js（顶层解构引入，调用点零改动）
const { detectOfficialBadge, injectVerifyCard } = globalThis.__YH_VERIFY__;
...
(function() {
  ...
  var observer = new MutationObserver(function(records) {
    ...
    scheduleEvaluate();
    // v2.7.1：AI 对话页外链扫描联动（模块 modules/content/link-scan.js，
    // 非可信内容平台时其内部自短路）
    if (globalThis.__YH_LINK_SCAN__) globalThis.__YH_LINK_SCAN__.scheduleLinkScan();
  });
