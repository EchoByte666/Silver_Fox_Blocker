...
// ===== 全局配置 =====
// 日志开关：DEBUG=true 时输出详细日志（普通页面 F12 控制台查看）
const DEBUG = false;
const LOG_PREFIX = '[银狐拦截] ';
...
// 硬编码高危域名兜底（与 background.js HARDCODED_DOMAINS 保持一致，修改需两处同步）
const HARDCODED_DOMAINS = ['noah-admin.site', 'page-admin.site'];

// ===== 域名匹配工具 =====
// 注意：这些函数与 background.js 保持相同语义，修改需两处同步

// 规则域名（v2.0.0 扩展）：
//   - 主域标签含连字符的 *.com.cn、*.hl.cn 及其所有子域名
//   - 主域标签含连字符的 *.cc 及其所有子域名
function matchesPatternDomain(hostname) {
  hostname = (hostname || '').toLowerCase().replace(/^\.+|\.+$/g, '');
  const labels = hostname.split('.');
  if (labels.length >= 3 && (labels[labels.length - 2] === 'com' || labels[labels.length - 2] === 'hl') &&
    labels[labels.length - 1] === 'cn' &&
    labels[labels.length - 3].includes('-')) return true;
  // *.cc 模式（v2.0.0 新增）
  if (labels.length >= 2 && labels[labels.length - 1] === 'cc' &&
    labels[labels.length - 2].includes('-')) return true;
  return false;
}
...
// 经典 Levenshtein DP 的特化版：在首个差异点处穷举三种单步操作，
// 任一能对齐剩余部分即距离 ≤1。两个短字符串（域名主体/品牌词）开销可忽略。
// 覆盖的 typo 形态：单字符插入（huorong→huorrong 双写）、
// 删除（huorrong→huorong 缺写）、替换（todesc→todesk）
function levenshteinWithin1(a, b) {
  ...
  if (i === lenA && j === lenB) return true;         // 完全相同
  // 在差异点穷举三种单步修复：a 删一字符 / b 删一字符 / 替换
  var rest1 = i + 1 < lenA ? a.slice(i + 1) : '';
  var rest2 = j < lenB ? b.slice(j) : '';
  if (rest1 === rest2) return true;                  // a 删除一位后相同
  rest1 = i < lenA ? a.slice(i) : '';
  rest2 = j + 1 < lenB ? b.slice(j + 1) : '';
  if (rest1 === rest2) return true;                  // b 删除一位后相同
